...
public class SampleBankingAction extends AbstractAction {
  ...
  protected Workspace create(Workspace workspace) {
    ...
    // containers
    Container singlePageApplication =
        internetBankingSystem.addContainer(
    ...
    apiApplication.getComponents().stream()
        ...
        .forEach(c -> singlePageApplication.uses(c, "Makes API calls to", "JSON/HTTPS"));
    apiApplication.getComponents().stream()
        .filter(c -> "Spring MVC Rest Controller".equals(c.getTechnology()))
        .forEach(c -> mobileApp.uses(c, "Makes API calls to", "JSON/HTTPS"));
    ...
    ContainerInstance developmentDatabase =
        developerLaptop
            .addDeploymentNode(
                "Docker Container - Database Server", "A Docker container.", "Docker")
            .addDeploymentNode("Database Server", "A development database.", "Oracle 12c")
            .add(database);

    ContainerInstance developmentSinglePageApplication =
        developerLaptop
    ...
    ContainerInstance liveSinglePageApplication =
        customerComputer
            .addDeploymentNode("Web Browser", "", "Chrome, Firefox, Safari, or Edge")
            .add(singlePageApplication);
    ...
    DeploymentNode liveApiServer =
        bigBankDataCenter.addDeploymentNode(
            ...
            8,
            MapUtils.create("Location=London and Reading"));
    ContainerInstance liveApiApplication =
        liveApiServer
            .addDeploymentNode(
                "Apache Tomcat",
                "An open source Java EE web server.",
    ...
    DeploymentNode primaryDatabaseServer =
        bigBankDataCenter
            .addDeploymentNode(
                "bigbank-db01",
    ...
    DeploymentNode secondaryDatabaseServer =
        bigBankdb02.addDeploymentNode(
            "Oracle - Secondary",
    ...
    model.getRelationships().stream()
        .filter(r -> r.getDestination().equals(liveSecondaryDatabase))
    ...
    ComponentView componentView =
        views.createComponentView(
            apiApplication, "Components", "The component diagram for the API Application.");
    ...
    systemLandscapeView.addAnimation(
        internetBankingSystem, customer, mainframeBankingSystem, emailSystem);
    ...
    componentView.addAnimation(
        singlePageApplication, mobileApp, database, emailSystem, mainframeBankingSystem);
    ...
    DeploymentView liveDeploymentView =
        views.createDeploymentView(
            internetBankingSystem,
            "LiveDeployment",
